...
const PaymentVerify = () => {
  ...
  useEffect(() => {
    ...
    fetch(`${baseUrl}/api/tickets/paystack/verify/${reference}`)
      .then((res) => res.json())
      .then((data) => {
        if (data.success) {
          // Clear saved ticket selection — payment completed
          try { sessionStorage.removeItem("sus3_selectedTicket"); } catch (_) {}
          setTicketData(data);
          setStatus("success");
        } else {
          setStatus("error");
          ...
          <div className="rounded-2xl border border-red-200 bg-red-50 p-10 max-w-md w-full shadow-lg">
            ...
            <div className="bg-white rounded-xl p-4 text-left space-y-2 text-sm text-gray-700 border border-red-100">
              <p>
                ...
                <a
                  href="mailto:stepupsummit@gmail.com"
                  className="text-[#0B1F5C] underline"
                >
  ...
  const shareText = encodeURIComponent(
    `I just got my ticket to Step-Up Summit 3.0! 🎟️ Join me — register at https://stepupsummit.org`
  );
    ...
    <div className="min-h-screen bg-white flex flex-col">
      ...
      <div className="flex-1 flex flex-col items-center justify-center text-center px-5 py-20">
        <div className="rounded-2xl border border-green-200 bg-green-50 p-10 max-w-md w-full shadow-lg">
          ...
          <p className="text-gray-600 mb-6">
            Check your email for your ticket and QR code. See you at Step-Up
            Summit 3.0!
          </p>
